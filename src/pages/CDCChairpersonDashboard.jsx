...
const AppCard = ({ app }) => {
    ...
    const getGradient = () => {
        if (app.status === 'UNDER_REVIEW_HEAD') return 'bg-gradient-to-b from-amber-400 to-orange-500';
        if (app.status === 'READY_FOR_COLLECTION') return 'bg-gradient-to-b from-emerald-400 to-emerald-500';
        if (app.status === 'COLLECTED') return 'bg-gradient-to-b from-indigo-400 to-indigo-500';
        if (app.status.includes('REJECTED')) return 'bg-gradient-to-b from-rose-400 to-rose-500';
        return 'bg-gradient-to-b from-slate-300 to-slate-400';
    };

    return (
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-6 flex flex-col gap-6 hover:shadow-lg transition-all duration-300 overflow-hidden relative group">
                    ...
                    <div className="flex flex-col items-end gap-2">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${app.status.includes('REJECTED') ? 'bg-rose-50 text-rose-600 border-rose-100' :
                                app.status === 'COLLECTED' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                                    app.status === 'READY_FOR_COLLECTION' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                                        app.status === 'UNDER_REVIEW_HEAD' ? 'bg-amber-50 text-amber-600 border-amber-100' :
...
const CDCChairpersonDashboard = () => {
    ...
    useEffect(() => {
        document.title = 'RGIPT NOC — CDC Chairperson Dashboard';
        fetchApplications();
        fetchDepartments();
    }, []);
    ...
    const fetchApplications = async () => {
        try {
            setFetching(true);
            const res = await api.get('/cdc/applications');
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">
                            CDC Chairperson Overview
                        </h1>
                        <p className="text-slate-500 mt-1 font-medium">Read-only audit view of top-level NOC processing</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        {fetching ? (
                            ...
                            <EmptyState
                                ...
                                message="No applications are currently under TNP Head review."
                ...
                {activeTab === 'outbox' && (
                    <div className="space-y-6">
                        {fetching ? (
                            ...
                            <EmptyState
                                title="No Applications Ready"
                                message="No applications are currently ready for collection."
...
export default CDCChairpersonDashboard;
